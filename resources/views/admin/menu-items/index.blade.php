                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="category_id" class="form-select" required>
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}">{{ $category->name }} ({{ $category->station }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <tbody id="menuItemsBody">
                            @forelse($menuItems as $item)
                                <tr class="js-row js-anim-init" data-anim="row">
                                    <td>
                                        @if($item->image)
                                            <img src="{{ $item->image_url }}" alt="{{ $item->name }}" class="menu-thumb js-thumb">
                                        @else
                                    ...
                                    <td>
                                        <form action="{{ route('admin.menu-items.toggle', $item) }}" method="POST"
                                            ...
                                            @csrf @method('PATCH')
                                            <button
                                                class="btn btn-sm {{ $item->is_available ? 'btn-success' : 'btn-outline-secondary' }}">
                                    ...
                                    @if(auth()->user()->role == 'admin')
                                        <td class="text-end">
                                            ...
                                            <form action="{{ route('admin.menu-items.destroy', $item) }}" method="POST"
                                                style="display:inline;" data-confirm="Remove this item?" class="js-delete-form">
                                                @csrf @method('DELETE')
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <x-rich-text-editor name="description"
                                                            id="edit_description_{{ $item->id }}" :value="$item->description" />
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label">Allergy / Dietary Info</label>
                                                        <textarea name="allergy_info" class="form-control" rows="2"
                                                            placeholder="e.g. Contains peanuts, dairy. Can be made gluten-free.">{{ $item->allergy_info }}</textarea>
                                                        <small class="text-muted">Shown as a clear warning badge to guests and
                                                            kitchen staff.</small>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label">Price ($)</label>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        @endif
                                                        <input type="file" name="image" class="form-control" accept="image/*">
                                                        <small class="text-muted">Supported: JPEG, PNG, JPG, GIF, WEBP (Max
                                                            10MB)</small>
                                                    </div>
                                                </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4 js-anim-init" data-anim="empty-row">No
                                        menu items yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
